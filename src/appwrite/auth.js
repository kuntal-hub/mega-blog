...
export class AuthService{
    ...
    async createRecovery({email}){
        try {
            await this.account.createRecovery(email,'http://localhost:5173/recovery-confirmation');
            return true;
        } catch (error) {
    ...
    async createEmailVeryfication(){ 
        try {
            await this.account.createVerification('http://localhost:5173/confirm-verification');
            return true
        } catch (error) {
